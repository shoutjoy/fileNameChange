...
- 하위 폴더명까지 함께 변경 가능
- **for문을 이용하여 폴더명과 숫자들을 결합한 폴더 일괄 생성 기능 (예: AI_과제_01 ~ AI_과제_15)**
...
2. 하위 폴더 안의 파일까지 변경하지 않으려면 `하위 폴더 포함` 체크를 해제합니다.
3. 폴더명도 같이 변경하려면 `폴더명도 변경`을 체크합니다.
4. 원하는 변경 방식을 선택합니다.
5. 실행 결과는 하단 로그 영역에서 확인합니다.

### 2. 접두사/접미사 추가

선택한 폴더의 모든 파일 이름에 접두사 또는 접미사를 추가합니다.

기본값은 `하위 폴더 포함`입니다. 선택한 폴더 아래의 모든 하위 폴더 파일도 함께 처리합니다.

`폴더명도 변경`을 체크하면 하위 폴더 이름에도 같은 접두사와 접미사를 적용합니다. 선택한 대상 폴더 자체의 이름은 변경하지 않습니다.
...
기본값은 `하위 폴더 포함`입니다. 하위 폴더의 파일까지 전체 경로 기준으로 정렬한 뒤 순서대로 번호를 붙입니다.

`폴더명도 변경`을 체크하면 하위 폴더 이름은 `기본이름_folder_001` 형식으로 변경합니다. 선택한 대상 폴더 자체의 이름은 변경하지 않습니다.
...
`폴더 일괄 생성` 탭을 선택하면, 지정한 대상 폴더(부모 폴더) 내에 일련번호가 결합된 폴더들을 일괄 생성할 수 있습니다. 
입력 요소는 **접두사(Prefix), 폴더명(Name), 시작값(From), 끝값(To), 접미사(Tailfix)**이며, 번호의 자릿수(Padding)를 지정할 수 있어 유연하게 폴더를 설계할 수 있습니다.
...
- 접미사 (Tailfix): `` (공백)
- 숫자 자릿수 (Padding): `2`
- 생성 결과: `AI_과제_01` ~ `AI_과제_15` 까지의 폴더 15개가 부모 폴더 내에 일괄 생성됩니다.
...
- `폴더명도 변경`을 체크하면 하위 폴더부터 순서대로 폴더명을 변경합니다.
- 적용 후 변화가 없다면 실행 결과 로그의 `대상 파일 n개 발견`과 `실패` 메시지를 확인합니다.
