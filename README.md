...
2. 하위 폴더 안의 파일까지 변경하려면 `하위 폴더 포함`을 체크합니다.
3. 원하는 변경 방식을 선택합니다.
4. 실행 결과는 하단 로그 영역에서 확인합니다.

### 2. 접두사/접미사 추가

선택한 폴더의 모든 파일 이름에 접두사 또는 접미사를 추가합니다.

`하위 폴더 포함`을 체크하면 선택한 폴더 아래의 모든 하위 폴더 파일도 함께 처리합니다.
...
`하위 폴더 포함`을 체크하면 하위 폴더의 파일까지 전체 경로 기준으로 정렬한 뒤 순서대로 번호를 붙입니다.
...
- `하위 폴더 포함`을 체크하지 않으면 선택한 폴더 바로 아래의 파일만 처리합니다.
